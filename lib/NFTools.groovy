...
""")
    ...
    """.stripIndent()}
      ...
      */
       ...
       * Format is: "idSample,sampleBarcode,normalBarcode,pathToDnaFastqR1,pathToDnaFastqR2,sampleDnaBam,sampleVcf,pathToRnaFastqR1,pathToRnaFastqR2,sampleRnaBam,sampleRnaBamIndex,hlaI"
       *
       * @param samplePlan
       * @return
       */
      ...
      public static Object getInputData(samplePlan, workflow) {
        ...
        if (samplePlan) {
	      ...
	      def meta = [:]
              ...
              def fastqDnaR1 = Nextflow.file(row[3])
              def fastqDnaR2 = Nextflow.file(row[4])
              def sampleDnaBam = Nextflow.file(row[5])
              def sampleDnaBamIndex = Nextflow.file(row[6])
              def vcf = Nextflow.file(row[7])
              def fastqRnaR1 = Nextflow.file(row[8])
              def fastqRnaR2 = Nextflow.file(row[9])
              def sampleRnaBam = Nextflow.file(row[10])
              def sampleRnaBamIndex = Nextflow.file(row[11])
              def hlaI = row[12]
              def hlaII = row[13]
              def tpmG = row[14]
              def tpmT = row[15]
              return [meta, fastqDnaR1, fastqDnaR2, sampleDnaBam, sampleDnaBamIndex, vcf, fastqRnaR1, fastqRnaR2, sampleRnaBam, sampleRnaBamIndex,hlaI,hlaII,tpmG,tpmT] 
